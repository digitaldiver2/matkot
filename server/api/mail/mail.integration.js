...
describe('Mail API:', function() {
  ...
  describe('GET /api/mail/:id', function() {
    var mail;

    beforeEach(function(done) {
      request(app)
        .get('/api/mail/' + newMail._id)
        .expect(200)
        .expect('Content-Type', /json/)
        .end((err, res) => {
          if (err) {
  ...
  describe('PUT /api/mail/:id', function() {
    var updatedMail;

    beforeEach(function(done) {
      request(app)
        .put('/api/mail/' + newMail._id)
        .send({
          name: 'Updated Mail',
  ...
  describe('DELETE /api/mail/:id', function() {

    it('should respond with 204 on successful removal', function(done) {
      request(app)
        .delete('/api/mail/' + newMail._id)
        .expect(204)
        .end((err, res) => {
          if (err) {
            return done(err);
